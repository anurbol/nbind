...
#define function(name, ...) nbind::FunctionDefiner NBIND_UNIQUE(definer, __LINE__)(#name, &name, ## __VA_ARGS__)

#define multifunction(name, args, ...) nbind::FunctionDefiner :: template Overloaded args NBIND_UNIQUE(definer, __LINE__)(#name, &name, ## __VA_ARGS__)
